...
final class LayoutManager: NSLayoutManager {
    ...
    override var delegate: NSLayoutManagerDelegate? {
        ...
        set {
            if !(newValue is LayoutManager) {
                log(
                    type: .error,
                    "LayoutManager delegate should not be modified to delegate: %@",
    ...
    override func drawGlyphs(forGlyphRange glyphsToShow: NSRange, at origin: CGPoint) {
        log(
            type: .debug,
            "range: %@, at: %@",
            NSStringFromRange(glyphsToShow),
            NSCoder.string(for: origin)
        )
        ...
        let signpostScanGlyphsToShow = signpost(
            name: "Scan glyphs to show",
            "length: %d",
            glyphsToShow.length
        )
        ...
        for index in 0..<glyphsToShow.length where properties[index].contains(.controlCharacter) {
            let attributes = textStorage.attributes(
                at: characterIndexes[index],
                effectiveRange: nil
            )
            if let suffixedAttachment = attributes[.suffixedAttachment] as? TextAttributes
                .SuffixedAttachment,
                case .image(let image) = suffixedAttachment.attachment
            {
                let glyphIndex = glyphsToShow.location + index
                let lineFragmentOrigin = lineFragmentRect(
                ...
                let locationInContext = CGPoint(
                    x: origin.x + lineFragmentOrigin.x + locationInLineFragment.x,
...
extension LayoutManager: NSLayoutManagerDelegate {
    ...
    private class CacheableGlyphs {
        ...
        init(mutableGlyphs: MutableGlyphs) {
            let glyphs = UnsafeMutableBufferPointer<CGGlyph>
                .allocate(capacity: mutableGlyphs.glyphs.count)
            _ = glyphs.initialize(from: mutableGlyphs.glyphs)
            self.glyphs = glyphs

            let properties = UnsafeMutableBufferPointer<NSLayoutManager.GlyphProperty>
                .allocate(capacity: mutableGlyphs.properties.count)
            _ = properties.initialize(from: mutableGlyphs.properties)
            self.properties = properties

            let characterIndexes = UnsafeMutableBufferPointer<Int>
                .allocate(capacity: mutableGlyphs.characterIndexes.count)
    ...
    func layoutManager(
        ...
        let unsafeBufferGlyphs = UnsafeBufferGlyphs(
            glyphs: glyphs,
            properties: props,
        ...
        for index in 0..<unsafeBufferGlyphs.count {
            ...
            {
                let insertion = MutableGlyphs.Insertion(
                    index: index + 1,
                    glyph: CGGlyph(0),
        ...
        layoutManager.setGlyphs(
            cacheableGlyphs.glyphs.baseAddress!,
    ...
    func layoutManager(
        ...
        log(
            type: .debug,
            "text container: %@, at end: %@",
            String(describing: textContainer),
            String(describing: layoutFinishedFlag)
        )
        ...
        for index in 0..<glyphsToShow.length where properties[index].contains(.controlCharacter) {
            ...
            {
                let glyphIndex = glyphsToShow.location + index
                let lineFragmentOrigin = layoutManager.lineFragmentRect(
                    forGlyphAt: glyphIndex,
                    effectiveRange: nil,
                    withoutAdditionalLayout: true
                ).origin
    ...
    func layoutManager(
        ...
        let attributes = textStorage.attributes(at: charIndex, effectiveRange: nil)
        guard let suffixedAttachment = attributes[.suffixedAttachment] as? TextAttributes
            .SuffixedAttachment
        else {
            // Should't reach here.
