...
final class TextView: UITextView {
    ...
    override func canPerformAction(_ action: Selector, withSender sender: Any?) -> Bool {
        ...
        if action == #selector(paste(_:)),
           let acceptableTypeIdentifiers = pasteConfiguration?.acceptableTypeIdentifiers
        {
            ...
            return pasteboardObserverCache.cached {
                ...
                if isPasteboardConformingAcceptableType {
                    if let delegate = pasteDelegate as? TextViewTextPasteDelegate {
                        ...
                        if let result = delegate.textPasteConfigurationSupporting?(
                            self,
                            canPaste: itemProviders
                        ) {
                            return result
                        }
                    }
    ...
    private let preferredTextInputModeContextIdentifier =
        ".TTETwitterTextEditorTextViewPreferredInputModeContextIdentifier"
    ...
    override func becomeFirstResponder() -> Bool {
        ...
        if result {
            if shouldWorkaroundTextInputModeBug {
                log(
                    type: .debug,
                    "Clear text input context identifier: %@",
    ...
    override var textInputContextIdentifier: String? {
        if shouldWorkaroundTextInputModeBug, preferredTextInputModePrimaryLanguage != nil {
            log(
                type: .debug,
                "Use text input context identifier: %@",
                preferredTextInputModeContextIdentifier
            )
    ...
    override func setBaseWritingDirection(
        _ writingDirection: NSWritingDirection,
        for range: UITextRange
    ) {
        super.setBaseWritingDirection(writingDirection, for: range)

        textViewTextInputDelegate?.textView(
            self,
            didChangeBaseWritingDirection: writingDirection,
            forRange: range
        )
    }
