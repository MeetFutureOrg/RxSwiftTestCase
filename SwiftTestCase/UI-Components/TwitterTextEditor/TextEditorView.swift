...
public final class TextEditorView: UIView {
    ...
    private var updateTextAttributesScheduler: ContentFilterScheduler<
        NSAttributedString,
        NSAttributedString?
    >!
    ...
    private var isUserInteractionBeingProcessed: Bool = false {
        didSet {
            log(
                ...
                "old value: %@, new value: %@",
                String(describing: oldValue),
                String(describing: isUserInteractionBeingProcessed)
            )
        }
    }
    ...
    private func update(with request: EditingContent.UpdateRequest) throws {
        ...
        let updated = editingContentDelegate?
            .textEditorView(self, updateEditingContent: interim) ?? interim
    ...
    private func scheduleUpdateTextAttributes() {
        ...
        updateTextAttributesScheduler.schedule(attributedString) { [weak self] result in
            ...
            guard case .success(let output) = result, let updatedAttributedString = output else {
                log(
                    type: .debug,
                    "Cancel update text attributes: %@, result: %@",
                    attributedString.loggingDescription,
            ...
            do {
                log(
                    type: .debug,
                    "Set text attributes: %@",
                    updatedAttributedString.loggingDescription
                )
                ...
                log(
                    type: .error,
                    "Failed to set text attributes error: %@",
    ...
    public private(set) var inputPrimaryLanguage: String? {
        didSet {
            if inputPrimaryLanguage != oldValue {
                textInputObserver?.textEditorView(
                    self,
                    didChangeInputPrimaryLanguage: inputPrimaryLanguage
                )
            }
        }
    }
    ...
    private func preparePlaceholderTextView() {
        if placeholderText != nil {
            ...
            NSLayoutConstraint.activate([
                ...
                placeholderTextView.leadingAnchor
                    .constraint(equalTo: textView.textInputView.leadingAnchor),
                placeholderTextView.trailingAnchor
                    .constraint(equalTo: textView.textInputView.trailingAnchor)
            ])
    ...
    private func updatePlaceholderTextView() {
        ...
        placeholderTextView.textContainer.lineFragmentPadding = textView.textContainer
            .lineFragmentPadding

        placeholderTextView.textContainer.lineBreakMode = placeholderTextLineBreakMode
        placeholderTextView.textContainer
            .maximumNumberOfLines = maximumNumberOfLinesForPlaceholderText
...
private extension NSMutableAttributedString {
    ...
    func setAttributes(from attributedString: NSAttributedString) throws {
        guard string == attributedString.string else {
...
extension TextEditorView: UITextViewDelegate {
    ...
    private func userInteractionDidChangeTextView() {
        ...
        guard let changeResult = textView.editingContent.changeResult(from: previousEditingContent)
        else {
            return
        }
...
extension TextEditorView: TextViewTextPasteDelegate {
    public func textPasteConfigurationSupporting(
        ...
        canPaste itemProviders: [NSItemProvider]
    ) -> Bool {
        for itemProvider in itemProviders {
            for observer in pasteObserversWithDefaults
                where observer.canAcceptItemProvider(itemProvider)
            {
    ...
    public func textPasteConfigurationSupporting(
        ...
        if acceptingObservers.count == 1, let observer = acceptingObservers.first {
            observer.transformItemProvider(
                itemProvider,
                completion: PasteObserverTransformCompletion { result in
