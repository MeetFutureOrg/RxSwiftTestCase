//
//  ASReorderTableViewController.m
//
...
/*
	...
	Implementation Overview
		...
		In addition to dragging a cell, ASReorderTableViewController will
...
@interface ASReorderTableViewController () {
...
@implementation ASReorderTableViewController
...
- (void)viewDidLoad {
	 ...
	 */
	__weak ASReorderTableViewController *blockSelf = self;
	if (resignActiveObserver == nil )
		resignActiveObserver = [[NSNotificationCenter defaultCenter] addObserverForName:UIApplicationWillResignActiveNotification object:nil queue:nil usingBlock:^(NSNotification *arg1) {
			if ( [blockSelf isDraggingCell] ) {
				ASReorderTableViewController *strongBlockSelf = blockSelf;
