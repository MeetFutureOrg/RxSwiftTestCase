...
enum DeviceMetrics: CaseIterable, Equatable {
    case iPhone4
    case iPhone5
    case iPhone6
    case iPhone6Plus
    ...
    static var allCases: [DeviceMetrics] {
        return [
            .iPhone4,
            .iPhone5,
            .iPhone6,
    ...
    init(
        ...
        onScreenNavigationHeight: CGFloat?
    ) {
        ...
        for device in DeviceMetrics.allCases {
            if onScreenNavigationHeight != nil, device.onScreenNavigationHeight(
                inLandscape: false,
            ...
            if (screenSize.width.isEqual(to: width) && screenSize.height.isEqual(to: height)) ||
                (
                    additionalSize.width.isEqual(to: width) && additionalSize.height
                        .isEqual(to: height)
                )
            {
    ...
    var type: DeviceType {
        ...
        case .iPad, .iPad102Inch, .iPadPro10Inch, .iPadPro11Inch, .iPadPro, .iPadPro3rdGen:
            return .tablet
        case .unknown(let screenSize, _, _)
            where screenSize.width >= 768.0 && screenSize.height >= 1024.0:
            return .tablet
        default:
            return .phone
        }
    ...
    var screenCornerRadius: CGFloat {
        ...
        case .unknown(_, _, let onScreenNavigationHeight):
    ...
    func safeInsets(inLandscape: Bool) -> UIEdgeInsets {
        ...
        case .iPhoneX, .iPhoneXSMax, .iPhoneXr, .iPhone12Mini, .iPhone12, .iPhone12ProMax:
            return inLandscape ? UIEdgeInsets(top: 0.0, left: 44.0, bottom: 0.0, right: 44.0) :
                UIEdgeInsets(
                    top: 44.0,
                    left: 0.0,
                    bottom: 0.0,
    ...
    func onScreenNavigationHeight(
        inLandscape: Bool,
        systemOnScreenNavigationHeight: CGFloat?
    ) -> CGFloat? {
        switch self {
        case .iPhoneX, .iPhoneXSMax, .iPhoneXr, .iPhone12Mini, .iPhone12, .iPhone12ProMax:
        ...
        case .iPad, .iPadPro, .iPadPro10Inch:
            if let systemOnScreenNavigationHeight = systemOnScreenNavigationHeight,
               !systemOnScreenNavigationHeight.isZero
            {
                return 21.0
            } else {
                return nil
    ...
    var statusBarHeight: CGFloat {
        ...
        case .iPadPro11Inch, .iPadPro3rdGen:
            return 24.0
        case .unknown(_, let statusBarHeight, _):
    ...
    func keyboardHeight(inLandscape: Bool) -> CGFloat {
        if inLandscape {
            switch self {
    ...
    func predictiveInputHeight(inLandscape: Bool) -> CGFloat {
        if inLandscape {
            switch self {
            case .iPhone4, .iPhone5, .iPhone6, .iPhone6Plus, .iPhoneX, .iPhoneXSMax, .iPhoneXr,
            ...
            case .iPhone4, .iPhone5:
                return 37.0
            case .iPhone6, .iPhoneX, .iPhoneXSMax, .iPhoneXr, .iPhone12Mini, .iPhone12,
                 .iPhone12ProMax:
                return 44.0
            case .iPhone6Plus:
                return 45.0
