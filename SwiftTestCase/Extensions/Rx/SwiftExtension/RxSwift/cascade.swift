...
public extension Observable where Element: ObservableType {
    /**
     ...
     */
    static func cascade<Sequence: Swift.Sequence>(_ observables: Sequence)
        -> Observable<Element.Element> where Sequence.Element == Element
    {
...
public extension ObservableType {
    /**
     ...
     - returns: An observable sequence that contains elements from the latest observable sequence that emitted elements
     */
    func cascade<S: Sequence>(_ next: S) -> Observable<Element> where S.Element == Self {
        return Observable.cascade([asObservable()] + Array(next).map { $0.asObservable() })
    }
}
