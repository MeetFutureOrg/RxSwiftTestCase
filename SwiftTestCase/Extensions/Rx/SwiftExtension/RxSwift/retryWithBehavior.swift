...
extension RepeatBehavior {
     ...
     */
    func calculateConditions(_ currentRepetition: UInt)
        -> (maxCount: UInt, delay: DispatchTimeInterval)
    {
        switch self {
        ...
        case .exponentialDelayed(let max, let initial, let multiplier):
            ...
            let delay = currentRepetition == 1 ? initial : initial * pow(
                1 + multiplier,
                Double(currentRepetition - 1)
            )
