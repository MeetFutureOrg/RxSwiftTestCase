...
open class RxViewModel: NSObject {
    ...
    @objc public dynamic var active: Bool {
        ...
        set {
            ...
            _active = newValue
            activeObservable.on(.next(_active))
        }
    }
    ...
    public func throttleSignalWhileInactive<T>(_ observable: Observable<T>) -> Observable<T> {
        ...
        let activeSignal = activeObservable
            .take(until: Observable.create { (_: AnyObserver<T>) -> Disposable in
        ...
        _ = Observable
            .combineLatest(activeSignal, observable) { active, obs -> (Bool?, T) in (active, obs) }
